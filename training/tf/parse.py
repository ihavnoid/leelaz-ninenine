...
class ChunkParser:
    ...
    def convert_train_data(self, text_item, symmetry):
        ...
        # Load the game winner color, and score if available
        winner_score = np.array(text_item[18].split()).astype(float)
        if len(winner_score) == 1:
            winner = winner_score[0]
            score = 0.0
        else:
            winner = winner_score[0]
            score = winner_score[1]
...
def generate_fake_pos():
    ...
    # 4. and a score random score
    score = [ 2 * float(np.random.randint(2)) - 1 ]
    return (planes, probs, winner, score)
...
def run_test(parser):
    ...
    planes, probs, winner, score = generate_fake_pos()
    ...
    # and finally a winner and score
    items.append(str(int(winner[0])) + " "  + str(float(score[0])) + "\n")
    ...
    with tf.Session() as sess:
        ...
        for symmetry in range(8):
            ...
            assert data == (sym_planes, sym_probs, winner, score)
...
def _parse_function(example_proto):
    ...
    return planes, parsed_features["probs"], parsed_features["legal"], parsed_features["winner"], parsed_features["score"]
